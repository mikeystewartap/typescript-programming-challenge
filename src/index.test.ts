...
import { solveSantaQuestion } from "./index";

describe("Input File: families.txt", () => {
  const inputFilePath = join(__dirname, "../assets/families.txt");

  it("returns the correct answer for the first question", async () => {
    const answer = await solveSantaQuestion(inputFilePath);
